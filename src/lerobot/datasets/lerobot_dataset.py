...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def add_frame_bytes(self, frame: dict, format: str | dict[str, str] | None = None) -> None:  # noqa: A002
        """Add a frame whose image/video features are already encoded bytes (or decoded arrays).

        Delegates to :meth:`DatasetWriter.add_frame_bytes`. The dataset must be in
        write mode (created via :meth:`create` or :meth:`resume`).
        ...
        Args:
            ...
            format: Explicit format override for byte payloads.
                None → sniff magic bytes per key. str → apply to all image keys. dict → per-key override.

        Raises:
            RuntimeError: If the dataset is read-only (no writer).
        """
        self._require_writer("add_frame_bytes")
        self.writer.add_frame_bytes(frame, format=format)
    ...
    def create(
        ...
        if (vcodec is not None or encoding_kwargs) and camera_encoder is None:
            _kwargs = dict(encoding_kwargs or {})
            _vcodec = vcodec or _kwargs.pop("vcodec", "libsvtav1")
            _pix_fmt = _kwargs.pop("pix_fmt", "yuv420p")
